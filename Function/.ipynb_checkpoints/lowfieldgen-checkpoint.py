...
class lowfieldgen(nn.Module):
    ...
    def forward(self, inParam):
        # print('Test')
        # print(type(inParam))
        # print('Shape k_high_T', inParam['k_high'].shape)

        (Nkx, Nky, Nkz, Nt, Ncoil, Comp) = tuple(inParam['k_high'].shape)
        ...
        if inParam['sequence'] == 'SpinEcho':
            fx = ((1 - E1_l) / (1 - E1_l * np.cos(theta))) / ((1 - E1_h) / (1 - E1_h * np.cos(theta)))*(E2_l / E2_h)
            scaleS = fx * (a ** 2)
        ...
        #print(((n_cov)).shape)
        d, v = torch.eig(n_cov[:,:,0], eigenvectors=True)   #d - eigenvectors, v - eigenvalues


        d = d[:,0] * torch.eye(8, dtype=torch.double)

        print('v shape', v.shape)
        # print('d shape', d)
        # print((v @ torch.sqrt(d)).shape)
        # print(torch.randn(Ncoil, Nkx * Nky * Nkz * Nt).shape)


        noise = scaleN * ((v @ torch.sqrt(d)) @ torch.randn(Ncoil, Nkx * Nky * Nkz * Nt, dtype=torch.double))
        print('noise shape',noise.shape)
        noise = noise.permute(1,0)
        noise = torch.reshape(noise, (Nkx, Nky, Nkz, Nt, Ncoil))
